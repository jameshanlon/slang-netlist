...
NB_MODULE(pyslang_netlist, m) {
  ...
  nb::class_<netlist::NetlistGraph>(m, "NetlistGraph")
      ...
      .def(
          ...
          "(2) run `VisitAll` to force lazy AST construction, "
          "(3) call `Compilation.freeze()`, "
          "(4) run `AnalysisManager.analyze()`, and "
          "(5) call `Compilation.unfreeze()` so the netlist builder can "
