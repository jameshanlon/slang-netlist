...
void DriverTracker::addDriver(SymbolDrivers &drivers,
  ...
  for (auto it = driverMap.find(bounds); it != driverMap.end();) {
    ...
    if (itBounds.first <= bounds.second && itBounds.second >= bounds.second) {
      ...
      driverMap.insert({bounds.first, bounds.second}, {DriverInfo(node, &lsp)},
                       mapAllocator);
      DEBUG_PRINT("Inserting new definition: [{}:{}]\n", bounds.first,
                  bounds.second);
...
auto DriverTracker::dumpDrivers(ast::Symbol const &symbol,
                                SymbolDriverMap &driverMap) -> std::string {
  FormatBuffer out;
